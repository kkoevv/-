...
with open("Time.txt", encoding="UTF-8") as file_in:
    time_ksp_before = (file_in.read()).split(', ')
with open("Hight.txt", encoding="UTF-8") as file_in:
    hight_ksp = [float(x) for x in (file_in.read()).split(', ')]
with open("Speed.txt", encoding="UTF-8") as file_in:
    speed_ksp = [float(x) for x in (file_in.read()).split(', ')]
with open("Mass.txt", encoding="UTF-8") as file_in:
    mass_ksp = [float(x) for x in (file_in.read()).split(', ')]

time_ksp = []
c = 0
for i in range(len(time_ksp_before)):
    time_ksp.append(c)
    c += 1
...
if __name__ == '__main__':
    X0 = [0.0, 0.0, 0.0, 0.0]
    t_end = 234.0
    t_points = np.linspace(0, t_end, 234)
    ...
    speed_ksp = speed_ksp[:234]
    mass_ksp = mass_ksp[:234]
    hight_ksp = hight_ksp[:234]

    otklon_mass = (sum([abs(mass_ksp[i] - mass_sol[i]) for i in range(len(mass_ksp))]) /
                   len([abs(mass_ksp[i] - mass_sol[i]) for i in range(len(mass_ksp))]))
    otklon_speed = (sum([abs(speed_ksp[i] - v_sol[i]) for i in range(len(speed_ksp))]) /
                   len([abs(speed_ksp[i] - v_sol[i]) for i in range(len(speed_ksp))]))
    otklon_hight = (sum([abs(hight_ksp[i] - y_sol[i]) for i in range(len(hight_ksp))]) /
                   len([abs(hight_ksp[i] - y_sol[i]) for i in range(len(hight_ksp))]))

    print(f'{round(otklon_mass, 3)} - среднее отклонение массы')
    print(f'{round(otklon_hight, 3)} - среднее отклонение высоты')
    print(f'{round(otklon_speed, 3)} - среднее отклонение скорости')


    fig, axs = plt.subplots(3, 3, figsize=(12, 10))

    axs[0][0].plot(t_points, speed_ksp, color='r', label='ksp')
    ...
    axs[0][1].plot(t_points, mass_ksp, color='r', label='ksp')
    ...
    axs[1][0].plot(t_points, hight_ksp, color='r', label='ksp')
